...
export default function Berries() {
  ...
  const fetchBerries = async (currentOffset) => {
    try {
      if (currentOffset === 0) setLoading(true);
      else setIsLoadingMore(true);

      const res = await fetch(`https://pokeapi.co/api/v2/berry?limit=50&offset=${currentOffset}`);
      const data = await res.json();
      
      const berryPromises = data.results.map(async (berry) => {
        const berryRes = await fetch(berry.url);
        return berryRes.json();
      });

      const newBerries = await Promise.all(berryPromises);
      
      setBerries(prev => {
        const existingIds = new Set(prev.map(i => i.id));
        const filteredNew = newBerries.filter(i => !existingIds.has(i.id));
  ...
  useEffect(() => {
    if (searchTerm) {
      setFilteredBerries(berries.filter(b => b.name.toLowerCase().includes(searchTerm.toLowerCase())));
    } else {
